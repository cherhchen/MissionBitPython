...
def buttonClicked(date_entry, category_entry, item_entry, expense_entry):
    ...
    if expense_entry.strip() != '' and expense_entry.isdecimal() == True:
        ...
        var = StringVar()
        stats_label = Label(stats_frame, relief=RAISED, textvariable=var, bg='#bfe9f5', font=('Courier', 1))
        ...
        stats_label.pack(fill=BOTH)

        totalvar = StringVar()
        total_label = Label(stats_frame, relief=RAISED, textvariable=totalvar, bg='#bfe9f5', font=('Courier', 1 ,'bold'))
        ...
        errorMessage = tk.Label(errorFrame, text='Error: Please input the appropriate values.', font=('Courier', 1), fg='red')
...
class AnEntry():
    ...
    def displayDate(self):
        label = tk.Label(scrollable_frame, text=self.date, font=('Courier', 2), relief=RAISED)
        #print(self.date, self.category, self.name, self.expense)
        return label

    def displayCategory(self):
        label = tk.Label(scrollable_frame, text=self.category, font=('Courier', 2), relief=RAISED)
        return label
    
    def displayName(self):
        label = tk.Label(scrollable_frame, text=self.name, font=('Courier', 2))
        return label
    
    def displayExpense(self):
        label = tk.Label(scrollable_frame, text=self.expense, font=('Courier', 2))
        return label
...
title_img = tk.PhotoImage(file='smallpiggybanner.gif')
...
cal = DateEntry(lower_frame, background='white', foreground='black', selectforeground='red', font=('Arial', 1))
cal.place(relx=0.025,rely=0.4, relwidth=0.2)
date_label = tk.Label(lower_frame, text="Date:", font=('Courier', 1), bg='#C6FFBD')
...
category_label = tk.Label(lower_frame, text="Category:", font=('Courier', 1), bg='#C6FFBD')
...
item_label = tk.Label(lower_frame, text="Item Name:", font=('Courier', 1), bg='#C6FFBD')
...
expense_label = tk.Label(lower_frame, text="Expense($):", font=('Courier', 1), bg='#C6FFBD')
...
#NAV BAR
def go_about():
    about_frame.place(rely=0.1, relwidth=1, relheight=0.9)
...
# About Button
about_frame = tk.Frame(root, bg= '#32739c')
about_button = Button(root, text="About", command=go_about, font=('Arial', 1))
about_button.place(relx=0.85, rely=0.05, relwidth=0.05, relheight=0.05)
about_moneycap = Text(about_frame, background= 'gray', highlightthickness= 0)
about_moneycap.insert(INSERT, "Moneycap is poggers. dingdong aflkldakf ad;lfd;fl adlkfakfk apkdfakdfk kdjfkajd;fkaj adfjadk;fjkad. adfkjadklfjadkfm adfkadlfk ad;kfad;jf ||| adf adkgl;gkdajgkjhgjk ")
about_moneycap.insert(END, "That is all.")
about_moneycap.pack(pady = 50)


# This is for the positioning of the text contained in the about frame

# Home Button
home_button = Button(root, text="Home", command=go_home, font=('Arial', 1))
home_button.place(relx=0.8, rely=0.05, relwidth=0.05, relheight=0.05)

# More info Button
moreinfo_frame = tk.Frame(root, bg= '#239e5c')
moreinfo_button = Button(root, text="More Info", command=go_moreinfo, font=('Arial', 1))
moreinfo_button.place(relx=0.9, rely=0.05, relwidth=0.05, relheight=0.05)
...
link1 = Label(moreinfo_frame, text = "50/30/20 rule", fg = "blue", cursor = "hand2")
link1.pack()
...
link2 = Label(moreinfo_frame, text = "Budgeting for college students", fg = "blue", cursor = "hand2")
link2.pack()
